...
**Pacing:** the **last 4 clips are held longer** (3.5s vs 2.0s) — those are usually the
finished-product shots, so they get more screen time. Upload the finished-product clips
last. Tune via `N_LONG` / `D_LONG` in `render-project.mjs` (and `render_reel.sh`).
