                            ...
                            <div key={job.id} style={card}>
                                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
                                    <div style={{ flex: 1 }}>
                                        ...
                                        {job.print_job_files?.map(f => (<div key={f.id} style={{ fontSize: '13px', color: '#4b5563', padding: '4px 0', display: 'flex', gap: '8px', flexWrap: 'wrap' }}><span>📄 {f.original_name}</span>{f.description && <span style={{ color: '#9ca3af' }}>— {f.description}</span>}<span style={{ color: '#9ca3af' }}>{f.width}×{f.height}{f.unit} Qty:{f.quantity}</span></div>))}
