...
export async function POST(req: Request) {
    ...
    const { files } = await req.json() as { files: Array<{ storagePath: string; originalName: string; displayName: string; description: string; width: number; height: number; unit: string; quantity: number }> };
    ...
    const fileRecords = files.map(f => ({ job_id: job.id, storage_path: f.storagePath, original_name: f.originalName, display_name: f.displayName || null, description: f.description, width: f.width, height: f.height, unit: f.unit, quantity: f.quantity }));
    ...
    try {
        await notifyAdminNewJob({
            ...
            files: files.map(f => ({ displayName: f.displayName, originalName: f.originalName, description: f.description, width: f.width, height: f.height, unit: f.unit, quantity: f.quantity })),
        });
