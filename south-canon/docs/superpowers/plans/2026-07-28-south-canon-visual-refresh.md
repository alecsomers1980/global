...
- **The existing Playwright e2e suite must pass unmodified** (`tests/e2e/*.spec.ts`) — in particular `play-detail.spec.ts`'s `'omits blocks that have no data'` test asserts *Saturday Night at the Palace* has **zero** `play_media` rows. Its `'shows availability by territory'` test asserts it has **zero** `rights_availability` rows (so it shows "Not currently available"). Do not add `play_media` or `rights_availability` rows to *Saturday Night at the Palace* — only `hero_image_url`. All gallery/video/rights content goes on the 6 new plays instead.
