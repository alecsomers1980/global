...
- [~] **Day 5 (original)** — Inventory-aware campaign variants. Pipeline shipped (campaign generator reads `cars` from Everest's Supabase, attaches photos for the ~70% vehicle-bound posts). **Superseded by Day 5 (revised) below — the per-car catalogue approach duplicates the trigger flow.** See [day-5-inventory-aware-variants.md](./day-5-inventory-aware-variants.md) for the original design and what landed.
- [ ] **Day 5 (revised)** — AI lifestyle images + branded overlay for every campaign post. Lifestyle photography generated via Gemini 2.5 Flash Image (nano-banana), informed by inventory segments. Logo top-left + workspace badge bottom-centre via `sharp`. ~80% pure lifestyle posts; ~20% optionally soft-link to a specific car. See [day-5-revised-lifestyle-images.md](./day-5-revised-lifestyle-images.md).
