...
import { formatDateTime, PLATFORM_LABELS, PLATFORM_COLORS } from '@/lib/utils'

export default async function ApprovePage({ params }: { params: Promise<{ token: string }> }) {
    const { token } = await params
    const supabase = await createServerSupabaseClient()

    // Find posts matching this approval token
    ...
    const postZeroAny = posts[0] as any
    const workspaceName = (postZeroAny.workspaces as any)?.name

    return (
        <div className="min-h-screen bg-[#0a0a0f] text-[#e2e2f0]" style={{ fontFamily: 'var(--font-sans)' }}>
            {/* Client-facing Header (No Agency Nav) */}
                                ...
                                {post.status === 'pending_approval' && (
                                    <div className="mt-6 grid grid-cols-2 gap-3">
                                        <form action={async () => {
                                            ...
                                            // In full version: trigger resend email to agency with the comment
                                        }}>
