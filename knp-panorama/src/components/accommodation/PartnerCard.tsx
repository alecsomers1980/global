...
export function PartnerCard({ name, location, website, blurb, children }: PartnerCardProps) {
  return (
    <div className="grid gap-8 rounded border border-ink/10 bg-white p-8 md:grid-cols-2">
      <div>
        <h3>{name}</h3>
        <p className="mt-1 text-xs uppercase tracking-wide3 text-text/50">{location}</p>
        <p className="mt-4 text-sm leading-relaxed text-text/70 normal-case">{blurb}</p>
        <a
          href={website}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-1 mt-4 text-sm text-amber hover:text-amber/80 transition-colors"
        >
          Visit Website
      ...
      <div>
        <p className="mb-3 text-xs uppercase tracking-wide3 text-text/60">
          Check availability
        </p>
