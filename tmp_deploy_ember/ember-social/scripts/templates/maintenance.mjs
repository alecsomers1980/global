...
// Matches reference post #4 (WORTH KNOWING — "Full service history / Doesn't guarantee a good car.").

import sharp from 'sharp'
import { escXml, generateImage, compositeLogo, buildHeadlineSvg, LOGO_PATH, nextSlot } from './common.mjs'
import { maintenanceCaption, maintenanceHashtags } from './captions.mjs'
...
export async function renderMaintenance(car) {
    console.log(`  Maintenance: ${car.year} ${car.make} ${car.model}`)
    const baseBuf = await generateImage(buildPrompt())
    const meta = await sharp(baseBuf).metadata()
    const W = meta.width, H = meta.height
    const tip = pickTip(car)

    // Small WORTH KNOWING label centred above the magnifier
    ...
    return {
        image: finalBuf,
        caption: maintenanceCaption(car),
        hashtags: maintenanceHashtags(),
        scheduledAt: nextSlot('fri'),
    }
}
