...
Permissions are enforced **server-side in the Next.js app**, with Airtable table/field permissions set as a secondary safeguard (views limited to appropriate records).
...
**Airtable supplemental lock**:  
- Base share permissions: admins have creator access; others are limited via custom views that only expose rows matching their category/status privileges and restrict field editing.
- The API token used by the Next.js app is a personal access token with read/write metadata; actual permissions are re-checked by the app on each request.
