...
**Airtable remains the database.** The application reads and writes exclusively through the Airtable Web API. This satisfies the requirement to be “built on Airtable” in the most literal sense: the Airtable base is the system-of-record for every ticket request, approval, guest list entry and report. Critically, Maynardville owns the Airtable account and pays for it directly. All records are stored in your base, and your team can open Airtable at any time and work with the data directly—a powerful fallback that no middleware-dependent solution can offer.
...
|---|----------|---------------|
| 1 | Which Airtable plan do you recommend and why? | The **Airtable Team plan** ($20 per seat/month, billed annually) includes the Web API, ample record limits and revision history. Only staff who actively edit are billable seats. We anticipate ~3 billable seats initially (Jaco, Wessel, Jeff); requesters and view-only department heads cost nothing. The **Business plan** is a future option if SAML/SSO or advanced admin controls become necessary. |
...
| 3 | How will permissions and role-based access be enforced? | Permissions are enforced in the application’s API layer on every request. A user’s role is determined at sign-in; the server rejects any attempt to read or write data outside the allowed scope. For extra safety, Airtable’s native share/view permissions can be set as a secondary lock. Requesters never have Airtable access at all. |
...
| 10 | What are the costs: one-off build and ongoing running costs? | **One-off build fee:** **[BUILD FEE — to be confirmed]** (proposed payment: 40% deposit, 30% on pilot acceptance, 30% on final handover). **Monthly running costs** are detailed in Section 9; they total approximately **R1,500/month** and are paid directly to Airtable and Vercel by Maynardville. |
| 11 | What does ongoing support and maintenance look like? | We provide thorough documentation and a training session so your team is self-sufficient for day-to-day operations. A post-handover support retainer can be arranged for enhancements, Quicket API changes, and season rollover assistance. Direct Airtable access remains available as a fallback, reducing reliance on external support. |
...
|---------|------|-------------------|----------------------------|-------|
| Airtable | Team plan (3× $20/seat) | $60 | ~R1,110 | Requesters and view-only users are free. Scale-up when additional editors are needed. |
