...
export default function EditEventPage() {
  ...
  const [originalImages, setOriginalImages] = useState<{ url: string; is_primary: boolean }[]>([]);
  ...
  async function fetchEvent() {
    ...
    if (error) {
      ...
      setFormData({
        ...
        images: data.images || [],
        ticket_options: data.ticket_options || [],
        slug: data.slug || '',
        booking_info: data.booking_info || { type: 'phone', value: '+27137900000' },
      });
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          const MAX_WIDTH = 1200; // Posters need bit better quality than avatars
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // 1. Cleanup removed images from storage
      const removedImages = originalImages.filter(orig => 
        !formData.images.some(current => current.url === orig.url)
      );
  ...
  if (loading) return (
    <div className="admin-page flex items-center justify-center py-20">
      <div className="flex flex-col items-center gap-4 opacity-50">
        ...
        <span className="text-xs font-bold uppercase tracking-widest text-brand-green">Loading Content...</span>
      </div>
    </div>
  );
    ...
    <div className="admin-page">
      ...
      <div className="flex justify-between items-center mb-10">
        <div>
          ...
          <h2 className="text-4xl font-black text-brand-green">Edit Event</h2>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="form-container">
            ...
            <label>Event Title</label>
             <input
              ...
              onChange={e => {
                ...
                // Only auto-generate slug if it's empty or we're creating a new one
        ...
        <div className="actions">
          ...
          <button type="submit" className="btn-main btn-save" disabled={saving}>
            {saving ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : 'Update Event'}
          </button>
        </div>
      </form>
    </div>
