...
interface Section {
  ...
  author?: string;
  icon?: string;
  extra_data: any; // Used for dates list, event details, etc.
}
...
export default function NewNewsletterPage() {
  ...
  const [sections, setSections] = useState<Section[]>([
    { id: '1', type: 'head', title: 'From the Head\'s Desk', body: '', image_url: '', author: 'Mr. Murray Johnson', icon: 'heart', extra_data: {} },
  ]);
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>, sectionId: string) {
    ...
    const loadingToastId = 'upload-' + Date.now();
  ...
  async function handlePublish(e: React.FormEvent) {
    ...
    if (sections.length > 0 && newsletter) {
      const sectionData = sections.map((s, i) => {
        // Clean up internal metadata like "uploading"
  ...
  return (
    <div className="pb-24">
      <style suppressHydrationWarning>{`
        ...
        .card-header { font-size: 14px; font-weight: 800; letter-spacing: 2px; text-transform: uppercase; color: #164e24; margin-bottom: 32px; display: flex; items-center gap: 12px; }
        ...
        .btn { padding: 16px 32px; border-radius: 12px; font-weight: 800; font-size: 15px; cursor: pointer; transition: all 0.2s; display: inline-flex; items-center gap: 8px; }
      ...
      <div className="max-w-[900px] mx-auto pt-8 mb-12">
        ...
        <h1 className="text-4xl font-extrabold text-[#1a2e1d] tracking-tight">Compose Newsletter</h1>
        <p className="text-[#1a2e1d]/50 mt-2 font-medium">Build the next edition of the Riverview Reporter</p>
      </div>
      ...
      <form id="newsletter-form" onSubmit={handlePublish} className="form-container">
        ...
        <div className="card">
          ...
          <div className="fg">
            ...
            <input required value={meta.title} onChange={e => setMeta({ ...meta, title: e.target.value })} placeholder="e.g. 12 March 2026 Edition" />
          </div>

          <div className="grid-2">
            <div className="fg">
              <label>Term</label>
              <input required value={meta.term} onChange={e => setMeta({ ...meta, term: e.target.value })} placeholder="e.g. Term 1" />
            </div>
            <div className="fg">
              <label>Issue Number</label>
              <input required value={meta.issue_number} onChange={e => setMeta({ ...meta, issue_number: e.target.value })} placeholder="e.g. Issue 05" />
            </div>
          </div>
          ...
          <div className="grid-2">
            <div className="fg">
              ...
              <input required value={meta.headline} onChange={e => setMeta({ ...meta, headline: e.target.value })} placeholder="e.g. Spring into Action: School Play & Victory" />
            </div>
            <div className="fg">
          ...
          <div className="fg">
            ...
            <textarea rows={2} value={meta.subheadline} onChange={e => setMeta({ ...meta, subheadline: e.target.value })} placeholder="A longer intro sentence that goes below the headline..." />
          </div>

          <div className="fg">
            <label>Highlights (Comma-separated labels)</label>
            <input value={meta.highlights} onChange={e => setMeta({ ...meta, highlights: e.target.value })} placeholder="e.g. Book Week, Swimming Medals, Golf Day" />
          </div>

          <div className="fg">
            <label>Excerpt (Used for listing cards)</label>
            <textarea rows={2} required value={meta.excerpt} onChange={e => setMeta({ ...meta, excerpt: e.target.value })} placeholder="Brief summary of what's inside this edition..." />
          </div>
        </div>
              ...
              <div className="section-body">
                <div className="grid-2 mb-6">
                  ...
                  <div className="fg mb-0">
                    ...
                    <input required value={section.title} onChange={e => updateSection(section.id, 'title', e.target.value)} placeholder="Title for this block" />
                  </div>
                </div>

                {/* Optional author/icon for specialised layout types */}
                {['head', 'academic', 'sport', 'preschool', 'content'].includes(section.type) && (
                  <div className="grid-2 mb-6">
                    <div className="fg mb-0">
                      <label>Author / Subtitle (Optional)</label>
                      <input value={section.author || ''} onChange={e => updateSection(section.id, 'author', e.target.value)} placeholder="e.g. Mr. Murray Johnson — Headmaster" />
                    </div>
                  </div>
                )}

                {/* Specific Fields for Achievement */}
                {section.type === 'achievement' && (
                  <div className="fg">
                    <label>Athlete / Student Name</label>
                    <input required value={section.extra_data?.athlete || ''} onChange={e => updateSection(section.id, 'extra_data', { ...section.extra_data, athlete: e.target.value })} placeholder="e.g. Usentele Sibiya" />
                  </div>
                )}

                {/* Specific Fields for Event Spotlight */}
                {section.type === 'event' && (
                  <div className="grid-2 mb-6">
                    <div className="fg mb-0">
                      <label>Event Subtitle</label>
                      <input required value={section.extra_data?.subtitle || ''} onChange={e => updateSection(section.id, 'extra_data', { ...section.extra_data, subtitle: e.target.value })} placeholder="e.g. Expect the Unexpected" />
                    </div>
                    <div className="fg mb-0">
                      <label>CTA Link</label>
                      <input value={section.extra_data?.ctaHref || ''} onChange={e => updateSection(section.id, 'extra_data', { ...section.extra_data, ctaHref: e.target.value })} placeholder="e.g. /events/oliver-twist" />
                    </div>
                  </div>
                )}

                {/* Content / Body — hide for 'dates' as they use list items */}
                {section.type !== 'dates' && (
                  <div className="fg">
                    <label>Content Body (Markdown supported)</label>
                    <textarea required rows={7} value={section.body} onChange={e => updateSection(section.id, 'body', e.target.value)} placeholder="Write the content here. Use paragraphs, bullet points, etc." />
                  </div>
                )}
                    ...
                    {(section.extra_data?.items || []).map((item: any, i: number) => (
                      <div key={i} className="flex gap-4 mb-4 items-start bg-white p-4 rounded-xl border border-slate-200">
                        ...
                        <div className="flex-1 space-y-3">
                          <input className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold outline-none focus:border-[#c4a459]" placeholder="Title (e.g. Rugby vs Uplands)" value={item.title || ''} onChange={(e) => {
                            const newItems = [...(section.extra_data.items || [])];
      ...
      <div className="floating-actions">
        <div className="actions-inner">
          ...
          <button type="submit" form="newsletter-form" className="btn btn-save" disabled={loading}>
            {loading ? <><Loader2 className="w-5 h-5 animate-spin" /> Publishing...</> : 'Publish Newsletter'}
          </button>
        </div>
      </div>
    </div>
