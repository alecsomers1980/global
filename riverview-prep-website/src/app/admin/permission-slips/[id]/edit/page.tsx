...
export default function EditPermissionSlipPage() {
  ...
  useEffect(() => {
    supabase.from('permission_slips').select('*').eq('id', id).single().then(({ data }) => {
      if (data) setForm({ title: data.title || '', description: data.description || '', event_date: data.event_date || '', due_date: data.due_date || '', status: data.status || 'draft' });
      setLoading(false);
    });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    setError('');
    const { error: updateErr } = await supabase.from('permission_slips').update({
      title: form.title,
    ...
    if (updateErr) { setError(updateErr.message); setSaving(false); }
    else { router.push('/admin/permission-slips'); }
  }
  ...
  return (
    <div>
      ...
      <div className="page-header">
        ...
        <h2 style={{ fontSize: 28, fontWeight: 800, color: '#1a2e1d' }}>Edit Permission Slip</h2>
      </div>
      ...
      <div className="form-card">
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            ...
            <input type="text" required className="form-input" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} />
          </div>
          <div className="form-group">
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input type="date" className="form-input" value={form.event_date} onChange={e => setForm(p => ({ ...p, event_date: e.target.value }))} />
            </div>
            <div className="form-group">
