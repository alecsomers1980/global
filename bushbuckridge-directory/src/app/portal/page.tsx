...
export default async function ClientPortalPage({
  ...
  const paymentStatus = typeof resolvedParams?.payment === 'string' ? resolvedParams.payment : null
  ...
  return (
    <div className="space-y-10">
      ...
      <div>
        <h1 className="text-4xl font-black tracking-tight text-primary">{business.name}</h1>
      ...
      {/* Upgrade section */}
      <UpgradeButton businessId={business.id} currentTier={business.package_tier || 'basic'} />
      ...
      {!hasAnalyticsAccess ? (
        <Card className="border-0 shadow-xl bg-card/60 backdrop-blur-xl rounded-[2rem] overflow-hidden text-center p-12 relative">
          ...
          <h2 className="text-3xl font-black text-primary mb-4">Analytics Locked</h2>
          <p className="text-muted-foreground text-lg max-w-md mx-auto mb-8 font-medium">
            Detailed analytics tracking is available on the Pro Lead and Pro Business packages. Upgrade above to unlock.
          </p>
        </Card>
      ) : (
