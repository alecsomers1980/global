...
export default async function AdminEventsPage({
  ...
  let events: any[] = []
  try {
    events = await pb.collection('events').getFullList({ sort: '-date' })
    if (q) {
              ...
              {events?.map((ev) => (
                <TableRow key={ev.id} className="hover:bg-primary/5 transition-colors border-primary/5">
                  ...
                  <TableCell className="py-6 px-4">
                    {ev.is_featured ? (
                      <Badge className="bg-amber-100 text-amber-700 hover:bg-amber-200 border-0 shadow-none font-bold">Featured</Badge>
                    ) : (
                      <span className="text-muted-foreground/30">-</span>
                    )}
                  </TableCell>
                  <TableCell className="py-6 px-4">
                    <EventActionsMenu event={ev} />
                  </TableCell>
                </TableRow>
              ))}
              {(!events || events.length === 0) && (
                <TableRow>
                  <TableCell colSpan={5} className="py-8 text-center text-muted-foreground font-medium">No events found.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
