...
import OppsClientWrapper from './OppsClientWrapper'
            ...
            <TableBody>
              {opps?.map((opp) => (
                <TableRow key={opp.id} className="hover:bg-primary/5 transition-colors border-primary/5">
                  ...
                  <TableCell className="py-6 px-4">
                    <OpportunityActionsMenu opportunity={opp} />
                  </TableCell>
                </TableRow>
              ))}
              {(!opps || opps.length === 0) && (
                <TableRow>
                  <TableCell colSpan={4} className="py-8 text-center text-muted-foreground font-medium">No opportunities found.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
