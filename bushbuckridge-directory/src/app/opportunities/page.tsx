...
import { Briefcase, AlertCircle, Download, ExternalLink, Sparkles, TrendingUp } from 'lucide-react'
                ...
                {error ? (
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
                        {opps?.map((opp) => {
                            ...
                            return (
                                <Card key={opp.id} className={`group flex flex-col overflow-hidden border-0 bg-card/50 backdrop-blur-sm shadow-xl transition-all duration-500 hover:shadow-2xl hover:-translate-y-3 rounded-[2.5rem] ${closed ? 'opacity-60 saturate-50' : ''}`}>
                                        ...
                                        {opp.deadline && (
                                            <div className="flex items-center gap-2 mt-6 p-4 bg-primary/5 rounded-2xl border border-primary/5">
                                                <div className="p-2 bg-white rounded-xl shadow-sm">
                                                    <AlertCircle className={`h-4 w-4 ${closed ? 'text-red-500' : 'text-primary'}`} />
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="text-[10px] font-black text-primary/40 uppercase tracking-widest">Deadline</span>
                                                    <span className={`text-sm font-bold ${closed ? 'text-red-500 line-through' : 'text-foreground'}`}>
                                    ...
                                    <CardContent className="px-8 pb-8 flex-1">
                                        <p className="text-muted-foreground font-medium italic border-l-2 border-primary/10 pl-4 text-sm leading-relaxed">
                                            {opp.contact_info}
                                        </p>
                                    </CardContent>
                                    <CardFooter className="p-8 pt-0 gap-4">
                                        ...
                                        <Button className="h-14 flex-1 bg-primary hover:bg-primary/90 rounded-2xl font-black shadow-lg shadow-primary/10 transition-all active:scale-95" disabled={closed}>
