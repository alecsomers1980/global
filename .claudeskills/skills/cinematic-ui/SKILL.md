---
description: Build high-fidelity, cinematic "1:1 Pixel Perfect" landing pages utilizing GSAP, Tailwind, and React.
---

# Cinematic Landing Page Builder Skill
...
When the user asks to build a site using the `cinematic-ui` skill, immediately ask **exactly these questions** using AskUserQuestion in a single call, then build the full site from the answers. Do not ask follow-ups. Do not over-discuss. Build.
...
- **Stack:** Require React 19, Tailwind CSS v3 (or 4), GSAP 3 (with ScrollTrigger plugin), Lucide React for icons.
...
6. Scaffold the project: `npx create-next-app` (or Vite), install deps, write all files.
