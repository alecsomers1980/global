...
export async function fetchWebsiteActivity({ curr, prev }) {
  ...
  const [
    ...
    countInWindow(client, "cars", "created_at", curr.start, curr.end),
    countInWindow(client, "cars", "created_at", prev.start, prev.end),
    sumCarsSold(client, curr.start, curr.end),
    sumCarsSold(client, prev.start, prev.end),
    countInWindow(client, "car_events", "created_at", curr.start, curr.end),
    countInWindow(client, "car_events", "created_at", prev.start, prev.end),
    leadBreakdown(client, curr.start, curr.end),
    leadBreakdown(client, prev.start, prev.end),
    countInWindow(client, "value_my_car_requests", "created_at", curr.start, curr.end),
    countInWindow(client, "value_my_car_requests", "created_at", prev.start, prev.end),
    countTestDrives(client, curr.start, curr.end),
    countTestDrives(client, prev.start, prev.end),
    countInWindow(client, "newsletter_subscribers", "created_at", curr.start, curr.end),
    countInWindow(client, "newsletter_subscribers", "created_at", prev.start, prev.end),
  ]);
