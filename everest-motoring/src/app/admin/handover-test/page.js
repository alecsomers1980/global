...
export default function HandoverTestPage() {
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            setStatus("Generating… this takes ~1–3 minutes. Polling…");
            pollRef.current = setInterval(async () => {
                ...
                if (p.status === "ready") {
                    ...
                    clearInterval(pollRef.current);
                    setStatus("Failed: " + (p.error || "unknown"));
                    setBusy(false);
                }
            }, 8000);
        } catch (err) {
        ...
        <div className="flex-1 w-full px-6 py-8">
            <div className="max-w-2xl mx-auto">
                ...
                <p className="text-slate-500 mt-1 mb-6">
                    Preview the post-sale Seedance styles on any photo — no sale record needed. Each
                    run is a real generation and costs video credits.
                </p>
                ...
                <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-2xl p-6 space-y-5 shadow-sm">
                    ...
                    <div>
                        ...
                        <select name="style" defaultValue="pixel_build" disabled={busy}
                    ...
                    <div>
                        ...
                        <input type="text" name="buyer_name" placeholder="e.g. John" disabled={busy}
                            className="w-full px-4 py-3 border border-slate-300 rounded-lg" />
                    </div>
                    <button type="submit" disabled={busy}
                        className="px-6 py-3 bg-primary hover:bg-primary-dark disabled:bg-slate-300 text-black font-bold rounded-lg w-full">
                ...
                {videoUrl && (
                    <div className="mt-6">
                        ...
                        <a href={videoUrl} target="_blank" rel="noreferrer" className="text-sm text-primary-dark font-bold underline mt-2 inline-block">
                            Open / download
                        </a>
                    </div>
                )}
