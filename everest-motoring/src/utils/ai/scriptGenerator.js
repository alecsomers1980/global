...
- BACKGROUND — STRICT: the background environment matches the reference image EXACTLY — same parking surface, same buildings, trees, sky, walls, and lighting as in the source photo. Do NOT invent a new environment, do NOT change the background geometry, do NOT insert different surroundings, do NOT add walls, signs, posters, or logos that are not already present in the reference image. Nothing in the background changes — the vehicle and its surroundings are preserved frame-for-frame from the source image; only the camera's slow push-in introduces motion.
...
`;
        ...
        return [
            { scene: 1, location: "exterior", visual_prompt: `The ${car.year} ${car.make} ${car.model} is parked completely stationary and motionless — wheels do not turn, body does not move. The background environment matches the reference image EXACTLY: same parking surface, same buildings, trees, sky, walls, and lighting as in the source photo. Do not invent a new environment, do not change the background geometry, do not insert different surroundings, do not add walls, signs, posters, or logos that are not already present in the reference image. Nothing in the background changes — the vehicle and its surroundings are preserved frame-for-frame from the source image. Continuous slow cinematic push-in already in motion on frame one — only the camera moves. ${motionRule} Preserve the lighting of the source image. AUDIO: ${cleanAudio}: '${hookLine}'` },
