...
- USE THE SOURCE PHOTO AS-IS: the entire frame — vehicle, number plate, ground, buildings, sky, lighting, every detail — is preserved exactly as in the uploaded reference image. Do NOT add, remove, or modify anything. Do NOT change the number plate text or graphics; the existing plate is correct as-is. Do NOT invent or insert walls, signs, posters, logos, or surroundings that are not already in the reference photo.
- Camera angle is LOCKED — same camera position, same perspective, same framing as the source image throughout the entire clip. Do NOT orbit the vehicle, do NOT pan, do NOT change angle, do NOT reveal sides of the car or background not visible in the source image.
- Camera motion: a slow, gentle zoom only — either a slow zoom-in toward the car or a slow zoom-out away from the car, on the same optical axis. The zoom is already in motion on frame one and continues smoothly to the final frame. No held frames, no other camera movement.
