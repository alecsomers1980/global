...
4. MOTION — every scene is a slow 2D Ken Burns zoom on the source reference image, already in motion on frame one and continuing smoothly to the last frame. No 3D camera, no push-in through space, no orbit, no pan, no parallax — the source image scales as a flat photograph.
...
- The source exterior image of the ${car.make} ${car.model}, animated by a slow Ken Burns zoom-in OR zoom-out. The car is parked and motionless. Every detail of the source image is preserved exactly as captured.
- AUDIO: "Clean, studio-quality South African English Female Voiceover with absolutely no background noise, static, hiss, interference, or ambient sound — voice only on a silent background: [Write a catchy 1-sentence hook introducing the make and model. MAX 11 words, aim for 8–10. Must fit comfortably in an 8-second clip at a calm, unhurried pace.]"
...
- AUDIO: "South African English Female Voiceover: [Write a 1-sentence voiceover mentioning 1 or 2 of its best technology features. MAX 11 words, aim for 8–10. Must fit comfortably in an 8-second clip at a calm, unhurried pace — say less rather than rush.]"
...
- AUDIO: "South African English Female Voiceover: [Write a 1-sentence voiceover mentioning its comfort or powertrain features. MAX 11 words, aim for 8–10. Must fit comfortably in an 8-second clip at a calm, unhurried pace — say less rather than rush.]"

Scene 4: Closer (Presenter added to source image)
- The source exterior image of the ${car.make} ${car.model}, with a partial figure of a female presenter added to ONE corner of the frame (lower-left OR lower-right). The presenter occupies a maximum of 25% of the frame width and is visible from chest-up only — close to the camera at the edge of the frame.
- The presenter has shoulder-length blonde hair and wears a plain tailored navy blue blazer over a plain white blouse. The blazer is completely plain — no logos, no embroidery, no badges, no text. She stands perfectly upright, looks directly into the camera with a warm smile, and her hands rest near chest height with subtle natural gestures only.
- The remaining 75%+ of the frame is pixel-identical to the source image — same vehicle, same back wall and any logos already on it, same ground, same surroundings, same lighting, all preserved exactly. The presenter is the only added element; everything else stays as in the source. Slow Ken Burns zoom-in OR zoom-out, same camera angle and framing as the source image.
- AUDIO: "Clean, studio-quality South African English Female Voiceover with absolutely no background noise, static, hiss, interference, or ambient sound — voice only on a silent background: [Write a unique, premium 1-sentence Call to Action urging the viewer to contact, visit, or enquire with Everest Motoring regarding this specific make/model. MAX 11 words, aim for 8–10. Must fit comfortably in an 8-second clip at a calm, unhurried pace — say less rather than rush. Avoid the word 'new'. Avoid the phrase 'test drive'.]"
...
`;
        ...
        const zoomRule = "The clip is the source reference image animated by a slow 2D Ken Burns zoom (zoom-in or zoom-out), already in motion on frame one and continuing smoothly to the last frame. The camera angle, position, perspective, framing, and lighting all match the source image exactly. Every pixel outside the zoom action stays identical to the source image.";
        ...
        return [
            ...
            { scene: 4, location: "exterior", visual_prompt: `The source exterior image of the ${car.make} ${car.model}, with ${presenterDesc.toLowerCase()} added to ONE corner of the frame (lower-left or lower-right). The presenter occupies a maximum of 25% of the frame width and is visible from chest-up only — close to the camera at the edge of the frame. She stands perfectly upright, looks directly into the camera with a warm smile, hands resting near chest height with subtle natural gestures. The remaining 75%+ of the frame is pixel-identical to the source image — same vehicle, same back wall and any logos already on it, same ground, same surroundings, same lighting. The presenter is the only added element; everything else stays as in the source. ${zoomRule} AUDIO: ${cleanAudio}: '${ctaLine}'` }
        ];
    }
}
