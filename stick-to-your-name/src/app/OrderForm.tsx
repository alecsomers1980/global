...
import { rand, DELIVERY_META } from '@/lib/designs';
import type { DeliveryOption } from '@/lib/designs';
import type { DesignRow, SettingsRow } from '@/lib/db';
...
function deliveryPriceCents(option: DeliveryOption, s: SettingsRow): number {
...
export default function OrderForm({ designs, settings }: OrderFormProps) {
  ...
  const totalCents =
    settings.set_price_cents +
    (bagTag ? settings.bagtag_price_cents : 0) +
    (deliveryOption ? deliveryPriceCents(deliveryOption, settings) : 0);
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      {/* Top nav */}
      <nav className="bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-end gap-4 text-sm">
        <button
          type="button"
          onClick={() => setTrackOpen(true)}
          className="inline-flex items-center gap-1.5 text-gray-600 hover:text-brand-pink font-medium"
        >
      ...
      <section className="py-12 px-4 text-center bg-white">
        ...
        <div className="mt-6 inline-block bg-brand-pink text-white font-semibold rounded-full px-6 py-2 text-lg">
          {rand(settings.set_price_cents)} per set
        </div>
            ...
            <fieldset>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  ...
                  <span className="font-semibold">Yes (+{rand(settings.bagtag_price_cents)})</span>
                </label>
                <label
                  className={`cursor-pointer border-2 rounded-2xl p-4 flex flex-col items-center justify-center transition ${
                    !bagTag
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6">
            ...
            <fieldset>
              ...
              <div className="space-y-4">
                {(Object.keys(DELIVERY_META) as DeliveryOption[]).map((key) => {
                  const option = DELIVERY_META[key];
                  const priceCents = deliveryPriceCents(key, settings);
                  const Icon =
                    key === 'collect'
                      ? Store
                      : key === 'pudo'
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6">
            ...
            <ul className="space-y-2 text-sm text-gray-700">
              <li className="flex justify-between">
                ...
                <span>{rand(settings.set_price_cents)}</span>
              </li>
              {bagTag && (
                <li className="flex justify-between">
                  <span>Bag tag</span>
                  <span>{rand(settings.bagtag_price_cents)}</span>
                </li>
              )}
              {deliveryOption && (
                <li className="flex justify-between">
                  <span>{DELIVERY_META[deliveryOption].label}</span>
                  <span>
                    {deliveryPriceCents(deliveryOption, settings) > 0
                      ? rand(deliveryPriceCents(deliveryOption, settings))
                      : 'Free'}
                  </span>
                </li>
              )}
