  ...
  return (
    <div className="p-8 space-y-6">
      ...
      {subsections.length > 0 && (
        <div>
          <h2 className="text-sm font-medium text-slate-700 mb-2">Subsections</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {subsections.map((sub) => (
              <Link
                key={sub.id}
                href={`/dashboard/branches/${branch.slug}/${section.slug}/${sub.slug}`}
                className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 hover:shadow-md"
              >
                <div className="text-base font-semibold text-slate-900">{sub.name}</div>
                <div className="text-xs text-slate-500 mt-1">Open subsection</div>
              </Link>
            ))}
          </div>
        </div>
              ...
              {documents.map((doc) => (
                <tr key={doc.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-right space-x-4">
                    ...
                    {canUpload && (
                      <DeleteDocumentButton documentId={doc.id} path={path} />
                    )}
                  </td>
                </tr>
              ))}
