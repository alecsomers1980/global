...
export async function GET(request: Request) {
  ...
  // Build the list of section IDs the user can access
  let accessibleSectionIds: string[] | null = null
  if (profile.role !== 'super_admin') {
    const { data: perms } = await supabase
      .from('permissions')
      .select('section_id')
      .eq('user_id', profile.id)
    accessibleSectionIds = (perms ?? []).map((p) => p.section_id)
  ...
  if (accessibleSectionIds) {
    query = query.in('section_id', accessibleSectionIds)
  }
