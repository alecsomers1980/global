...
/** Post-purchase "how did you find it?" email.
 *
 *  `products` is only the items this customer can still review — the cron has
 *  already dropped anything they've reviewed before, so we never ask twice.
 *  Each links straight to that product's reviews section.
 *
 *  No medical framing anywhere in the copy (docs/compliance-rules.md): we ask
 *  about their experience of the product, never whether it "worked" on a
 *  condition — the answer to that question is exactly what we can't publish. */
export function reviewRequest(
  ...
  const list = products
    .map(
      (p) =>
        `<li style="margin-bottom:6px;"><a href="${siteUrl}/product/${encodeURIComponent(
          p.slug
        )}#reviews" style="color:#1F5C3D;">${esc(p.title)}</a></li>`
    )
    .join("\n");
  ...
  const body = `
    <p>Hi ${esc(order.full_name) || "there"},</p>
    <p>Thank you for your recent order (${esc(
      order.order_number
    )}). We hope you&#39;re enjoying it.</p>
    <p>If you have a moment, would you share how you found ${
      single ? "it" : "them"
    }? Honest reviews help other customers choose, and they mean a great deal to us.</p>
    <ul style="padding-left:20px; margin:16px 0 0;">${list}</ul>
    ${button(`${siteUrl}/product/${encodeURIComponent(products[0].slug)}#reviews`, single ? "Write a review" : "Write a review")}
    <p style="color:#6B7280; font-size:13px;">Please describe your own experience of the product. Reviews can&#39;t include medical claims, and are checked before they appear.</p>
    <p style="color:#6B7280; font-size:13px;">If you&#39;d rather not get these, just reply to this email and we&#39;ll stop them.</p>`;

  return {
    subject: single
      ? `How are you finding your ${products[0].title}?`
      : `How was your recent order, ${esc(order.full_name).split(" ")[0] || "there"}?`,
    html: layout(body),
  };
}
